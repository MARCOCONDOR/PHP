<?php
    ...
    switch($request_method)
	{
		case 'GET':
			...
			{
				$TCodigo=intval($_GET["Codigo"]);
				$Semestre=$_GET["Semestre"];
				

				buscar_tablas($TCodigo,$Semestre);
			}
			else{
...
//             modificar_interno($id,$codigo);
    ...
    function buscar_tablas($TCodigo,$Semestre){
		...
		$query="SELECT THaber,TFecha_Pago,TVoucher,TConcepto FROM tabla WHERE DNI='".$TCodigo."' AND TConcepto IN ('Matrícula','Pensión: Primera Cuota','Pensión: Segunda Cuota','Pensión: Tercera Cuota','Pensión: Cuarta Cuota')   AND Semestre='".$Semestre."' ";
