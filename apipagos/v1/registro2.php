    ...
    switch($request_method)
	{
		case 'GET':
			...
			if(!empty($_GET["Codigo"]))
			{
				...
				$date=$_GET["date2"];
				$datofin=$_GET["datofin2"];
				$VOUCHER=$_GET["VOUCHER2"];
				$BF=$_GET["BF2"];
				$Concepto1=$_GET["Concepto2"];
				$DEBE=$_GET["DEBE2"];
				$HABER=$_GET["HABER2"];
...
//             modificar_interno($id,$codigo);
    ...
    function Insertar_Registro($Codigo,$Semestre_Seleccionado,$date,$datofin,$VOUCHER,$BF,$Concepto1,$DEBE,$HABER){
		...
		$query="UPDATE tabla  SET TFecha_Vencimiento='".$date."' ,TFecha_Pago='".$datofin."' , TVoucher='".$VOUCHER."' , TBole_Factura='".$BF."' , TDebe='".$DEBE."' , THaber='".$HABER."' 
				WHERE TCodigo= '".$Codigo."' AND  	Semestre= '".$Semestre_Seleccionado."' AND TConcepto= '".$Concepto1."' " ;
